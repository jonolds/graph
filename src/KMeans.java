...
public class KMeans {
	...
	static void kmeans(SparkSession ss) throws Exception {
	    ...
	    do {
	    	...
	    	List<Double> distances_per_it = assigned.mapToDouble(x->distance(x._2, cents.get(x._1))).collect();
			costs.add(distances_per_it.stream().mapToDouble(x->Math.pow(x, 2)).sum());
...
//		while (count< MAX_ITERS);
	...
	static void graph() {
		SwingUtilities.invokeLater(() -> {
			KMeansGraph graph = new KMeansGraph(costs);
